...
type ModalProps = {
  ...
  add: (name:string, phone:string, email:string, company:string, companyID:string, region:string, email_T_F:string) => void;
};

const AddExternalModal = ({
  modalVisible,
  setModalVisible,
  add
}: ModalProps) => {
  ...
  const [emailTrueInput, setEmailTrueInput] = useState(false);
  const [textTrueInput, setTextTrueInput] = useState(false);
        ...
        <ModalView contentContainerStyle={{ alignItems: 'center' }}>
          ...
          <SpacedView>
            ...
            <StandardButton
              bgColor={"#485A72"}
              color={"#A9936D"}
              onPress={() => add(nameInput, phoneInput, emailInput, companyInput, companyIDInput, regionInput, (emailTrueInput ? (textTrueInput ? 'Both' : 'Email') : (textTrueInput ? 'Text (SMS)' : '')))}
              title="Save"
              width="45%"
            />
          </SpacedView>
